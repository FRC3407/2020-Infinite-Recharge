...
public class Robot extends TimedRobot {
  ...
  private int rotation_channel = 0;
  ...
  //NOTE:this is not refrencing a gyroscopic sensor, it is refrencing a 
  //greek sandwitch!
  ...
  public void robotInit() {
    ...
    //Setup the joysticks for controlling the robot
  ...
  public void teleopPeriodic() {
    ...
    m_myRobot.arcadeDrive(-1*m_joystick.getX(), m_joystick.getY());    //drive the wheels using the joystick's X and Y channel values.

    //use the A button to shoot the ball, release to stop spinning.
    if(m_joystick.getRawButton(1)) {
      Shoot(shoot_speed);//if the button is pressed, set the shoot speed to the constant above.
    }
    else {
      Shoot(0);    //if the button isn't pressed, set the shoot speed to zero.
    }
  }
